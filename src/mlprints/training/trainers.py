...
class TeacherLoss:
    """One soft-label teacher-loss term consumed by CompositeCausalLMTrainer."""

    mode: str                         # must be in CompositeCausalLMTrainer.LOSS_MODES
    weight: float
    temperature: float = 1.0
    scale_by_t_squared: bool = False  # multiply the loss by temperature^2


class CompositeCausalLMTrainer(Trainer):
    """
    Causal LM trainer: (hard-label) SFT loss plus optional (soft-label) teacher losses.

    Total loss is `sft_weight * sft_loss + sum_i (teacher_losses[i].weight * teacher_loss_i)`.
    ...
    Teacher losses are always OFFLINE: precomputed top-k targets with fields
    (`topk_indices`, `topk_log_probs`, `topk_mask`) are read from each batch.
    The allowed modes are in `LOSS_MODES`.
    ...
    LOSS_MODES = {
        "teacher_ce",                  # cross-entropy CE(teacher_p, student_p)
        "teacher_positive_deviation",  # positive deviation ReLU(student_p - teacher_p)
    }
    ...
    def __init__(
        ...
        *args,
        ...
        teacher_losses: list[dict | TeacherLoss] | None = None,
        **kwargs,
    ):
        ...
        self.teacher_losses = (
            [t if isinstance(t, TeacherLoss) else TeacherLoss(**t) for t in teacher_losses]
        ...
        if self.sft_weight == 0 and not self.teacher_losses:
        ...
        if self.teacher_losses is not None:
            if missing_cols := self._TOPK_KEYS - set(self.train_dataset.column_names):
                raise ValueError(f"teacher_losses require dataset columns: {sorted(missing_cols)}")
            for term in self.teacher_losses:
                if term.mode not in self.LOSS_MODES:
                    raise ValueError(f"unsupported loss mode: {term.mode}")
    ...
    def compute_loss(self, model, inputs, return_outputs=False, **kwargs):
        ...
        if self.sft_weight != 0:
            loss = loss + self.sft_weight * self._sft_loss(inputs, outputs)

        if self.teacher_losses is not None:
            for teacher_loss in self.teacher_losses:
                loss = loss + teacher_loss.weight * self._teacher_loss(inputs, outputs.logits, teacher_loss)
    ...
    def _teacher_loss(self, inputs, logits, term):
        ...
        if term.mode == "teacher_ce":
            per_position = -(teacher_probs * student_log_probs).sum(dim=-1)
        elif term.mode == "teacher_positive_deviation":
