...
def generate_inverse_nucleus_signatures_batched(
    ...
    # if out_file is None:
    #     out_file = key_file.replace('.json', f'-inverse-nucleus-{model_name.replace("/", "-")}-response_length-{response_length}.json')    
    if 'instr' in model_name.lower():
        out_file = key_file.replace('.json', f'-inverse-nucleus-{model_name.replace("/", "-")}-nucleus_threshold-{nucleus_threshold}-response_length-{response_length}-use_chat_template-{use_instr_model}.json')
    else:
        out_file = key_file.replace('.json', f'-inverse-nucleus-{model_name.replace("/", "-")}-nucleus_threshold-{nucleus_threshold}-nucleus_k-{nucleus_k}-response_length-{response_length}.json')    
...
def generate_inverse_nucleus_signatures_batched_multi_response(
    ...
    # if out_file is None:
    #     out_file = key_file.replace('.json', f'-inverse-nucleus-{model_name.replace("/", "-")}-response_length-{response_length}.json')    
    out_file = key_file.replace('.json', f'-inverse-nucleus-{model_name.replace("/", "-")}-nucleus_threshold-{nucleus_threshold}-response_length-{response_length}-num_responses-{num_responses}.json')    
...
if __name__ == "__main__":
    ...
    if args.random_word_generation:
        ...
        if args.num_responses_per_fingerprint == 1:
            keys_path = generate_inverse_nucleus_signatures_batched(keys_path, args.output_file_path, args.inverse_nucleus_model, args.response_length, args.key_length, nucleus_threshold=args.nucleus_p, nucleus_k=args.nucleus_k, num_fingerprints=args.num_fingerprints, batch_size=32, use_instr_model=args.use_chat_template)
        else:
            keys_path = generate_inverse_nucleus_signatures_batched_multi_response(keys_path, args.output_file_path, args.inverse_nucleus_model, args.response_length, args.key_length, nucleus_threshold=args.nucleus_p, nucleus_k=args.nucleus_k, num_fingerprints=args.num_fingerprints, batch_size=32, num_responses=args.num_responses_per_fingerprint)
