...
def get_save_dir(config: dict):
    ...
    save_dir = os.path.join("experiments", config["algo"]["name"], f"{timestamp}")
    return save_dir
...
def generate_fingerprints(config: dict):
    ...
    assert (
        config["algo"]["name"] == "rofl" or config["algo"]["name"] == "perinucleus"
    ), "Only rofl and perinucleus are implemented!"
    ...
    # elif config["algo"]["name"] == "perinucleus":
        ...
        metas = []
        train = input(
            "Perinucleus requires training. Would you like to train now? (y/N): "
        )
        if train.lower() == "y":
            train_perinucleus(fps, **config["algo"]["training"])
        return fps, metas
